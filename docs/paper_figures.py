...
def fig02_decomposition(smb, trend, seasonal_12, resid, path=None):
    ...
    ax[0].plot(t, trend, color=C_ALT, lw=1.4, label="linear trend")
...
def fig03_transform(resid, g_resid, q_semi, q_emp=None,
                    sat_factors=None, sat_semi=None, sat_emp=None,
                    path=None):
    """
    ...
    sat_factors : (F,) requested band scale factors, e.g. [1,2,5,10,20]
    sat_semi    : (F,) realised variance ratio using the semi-parametric CDF
    sat_emp     : (F,) realised variance ratio using the empirical CDF
    """
    resid   = np.asarray(resid).ravel()
    ...
    if sat_factors is not None:
        f = np.asarray(sat_factors, dtype=float)
        ax[1, 1].plot(f, f, color=C_GREY, lw=0.8, ls="--",
                      label="ideal (1:1)")
        if sat_semi is not None:
            ax[1, 1].plot(f, sat_semi, "o-", ms=3, color=C_SYN,
                          label="semi-parametric")
        if sat_emp is not None:
            ax[1, 1].plot(f, sat_emp, "s-", ms=3, color=C_ALT,
                          label="empirical only")
        ax[1, 1].set_xscale("log"); ax[1, 1].set_yscale("log")
        ax[1, 1].set_xlabel("requested band factor $\\gamma_b$")
        ax[1, 1].set_ylabel("realised variance ratio")
