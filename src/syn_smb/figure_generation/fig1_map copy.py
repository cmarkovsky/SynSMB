...
BASINS = {
    "Pine_Island": {"color": "#0072B2", "label": "Pine Island"},
...
def plot_basin_map(
    ...
    gdf_basins = gpd.read_file(shp_path)
    gdf_regions = gpd.read_file("./data/ANT_Basins_IMBIE2_v1.6/ANT_Basins_IMBIE2_v1.6.shp", label="regions")
    ...
    gdf_basins.plot(color="lightgray", edgecolor="white", linewidth=0.5, ax=ax_main)
    gdf_regions.plot(color="None", edgecolor="black", linewidth=1, ax=ax_main)
    print(gdf_regions)
    # Label each region
    ...
    #     # Transform to data coordinates if needed
    #     centroid = DATA_CRS.transform_point(centroid.x, centroid.y, PROJ)
    #     ax_main.text(
    ...
    basins = gdf_basins[gdf_basins["NAME"].isin(BASINS.keys())]
    basins.plot(color=[BASINS[basin]["color"] for basin in basins["NAME"]], edgecolor="black", linewidth=0.5, ax=ax_main)
    if name_col not in gdf_basins.columns:
        available = [c for c in gdf_basins.columns if c != "geometry"]
        raise ValueError(
